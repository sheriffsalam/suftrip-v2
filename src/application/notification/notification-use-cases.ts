...
export class CancelNotification {
  ...
  async execute(
    ...
    if (await this.notifications.hasOperationIdempotencyKey(notificationId, 'CANCEL', idempotencyKey)) return notification.snapshot();
...
async function deliver(
  ...
  try {
    ...
    if (error instanceof InvalidTransitionError) {
      throw new ConflictError(`Notification cannot be ${operation.toLowerCase()}ed from its current status`);
    }
    throw error;
  }
